...
def unzip(zipped):# convert a zipped dictionary to new_params
...
def numpy_floatX(data):# convert input data into numpy array with a specific data type config.floatx
...
#generate random numpy#generate random numpy of shape(dim1,dim2) with values sampled uniformly
# from the range [left,right] then convert to type config.floatx



def load_embedding(options):# loads pretrained words embedding, this embeddings are stored in a dictionary with two keys,
    #keys: 'w' and 'w_tilde'
...
def init_params(options):
    ...
    # generate random weights for different layers
    params['W_emb'] = get_random_weight(inputDimSize + numAncestors, embDimSize)
    if len(options['embFile']) > 0: # if an embedding file is provided in the 'options'
...
def dropout_layer(state_before, use_noise, trng, prob): # applies dropout to the input state_before, based on prob
    proj = T.switch(use_noise,# A binary switch (Theano symbolic variable) that decides whether to apply dropout (1) or not (0).
                    (state_before * trng.binomial(state_before.shape, p=prob, n=1, dtype=state_before.dtype)),
                    state_before * 0.5) #trng: A Theano random number generator.
    return proj
# The function uses the T.switch function from Theano to apply dropout when use_noise is True (1) or return the input
# tensor scaled by a factor of 0.5 when use_noise is False (0).
...
def gru_layer(tparams, emb, options): # returns the
...
def generate_attention(tparams, leaves, ancestors):# generate attention weights for a given input.
...
def train_GRAM(
    ...
    ancestorsList = []
    for i in range(5, 0,
                   -1):  # An ICD9 diagnosis code can have at most five ancestors (including the artificial root) when using CCS multi-level grouper.
...
def parse_arguments(parser):
    ...
    parser.add_argument('--L2', type=float, default=0.001,
                        help='L2 regularization coefficient for all weights except Seq2Seq.ipynb (default value: 0.001)')
    parser.add_argument('--dropout_rate', type=float, default=0.5,
                        help='Dropout rate used for the hidden layer of Seq2Seq.ipynb (default value: 0.5)')
